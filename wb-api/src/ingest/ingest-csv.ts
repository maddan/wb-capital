...
function parseCSV(text: string) {
  ...
  return lines.map(line => {
    // basic CSV split; fine for our sample (no embedded commas/quotes)
...
async function main() {
  ...
  try {
    const res = await axios.post(`${API_BASE}/ingest/bulk`, payload, { headers: { 'Content-Type': 'application/json' }});
